...
//  单链表的函数成员 
	...
	Status MovePart(LinkList<ElemType>& target, const int& position);
	Status DeletePart();
...
template<class ElemType>
inline Status LinkList<ElemType>::MovePart(LinkList<ElemType>& target, const int& i)
{
	if (i < 1 || i > length)
		return RANGE_ERROR;
	Node<ElemType>* p, * q, *pparent = head;
	p = pparent->next;
	for (q = target.head; q->next != NULL; q = q->next);
	...
	pparent->next = NULL;
	q->next = p;
	target.length += length - i + 1;
	length = i - 1;
	return SUCCESS;
}

template<class ElemType>
inline Status LinkList<ElemType>::DeletePart()
{
	if (13 > length)
		return RANGE_ERROR;
	int i = length - 12;
	Node<ElemType>* p, * pparent = head;
	p = pparent->next;
	for (int count = 1; count < i; count++)
	{
		pparent = p;
